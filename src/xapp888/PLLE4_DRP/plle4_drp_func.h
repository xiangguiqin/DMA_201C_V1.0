...
//	Revision Notes:	4/17/17  Fixed LOCK settings
//   Revision Notes:  10/17/17 -  fixed incorrect phase shift reporting in some cases
//   Revision Notes:  11/30/17 -  Phase calculation rounding to nearest possible phase
//   Revision Notes:  12/18/17 -  Fixed incorrect calculations for fractional divide
//
...
function [39:0] pll_lock_lookup
   ...
   begin
      ...
      pll_lock_lookup = lookup[ ((21-divide)*40) +: 40];
   `ifdef DEBUG
...
function [9:0] pll_filter_lookup
   ...
   begin
      ...
      lookup = {
         ...
         pll_filter_lookup = lookup [ ((21-divide)*10) +: 10];

 ...
 *
 * @param     GPIOx: Select the GPIO GPIOx.
 *                  This parameter can be one of GPIOx( x can be from A to G).
 *
 * @retval    None
 */
void GPIO_Reset(GPIO_T* GPIOx)
{
 ...
 * @retval    None
 */
void GPIO_Config(GPIO_T* GPIOx, GPIO_Config_T* gpioConfig)
{
    uint8_t i;
    uint32_t mode;
    uint32_t CR;
    ...
    if (gpioConfig->GPIO_Pin & 0xff)
    {
        CR = GPIOx->CFGLOW;

        for (i = 0, shift = 0x01; i < 8; i++, shift <<= 1)
        {
            if (gpioConfig->GPIO_Pin & shift)
            {
                temp = i << 2;
...
/*!
 * @brief     Reads the specified input GPIOx GPIO_Pin
 *
 * @param     GPIOx: Select the GPIO GPIOx.
 *                  This parameter can be one of GPIOx( x can be from A to G).
 *
 * @param     GPIO_Pin : specifies GPIO_Pin to read.
 *                  This parameter can be one of GPIO_PIN_x( x can be from 0 to 15).
 *
 * @retval    The input GPIOx GPIO_Pin value
 */
uint8_t GPIO_ReadInputBit(GPIO_T* GPIOx, uint16_t GPIO_Pin)
{
    uint8_t ret;

    ret = (GPIOx->IDATA & GPIO_Pin) ?  BIT_SET : BIT_RESET;

    return ret;
}

/*!
 * @brief     Reads the specified GPIO input data GPIOx
 *
 * @param     GPIOx: Select the GPIO GPIOx.
 *                  This parameter can be one of GPIOx( x can be from A to G).
 *
 * @retval    GPIO input data GPIOx value
 */
uint16_t GPIO_ReadInputPort(GPIO_T* GPIOx)
{
    return ((uint16_t)GPIOx->IDATA);
}

/*!
 * @brief     Reads the specified output data GPIOx bit
 *
 * @param     GPIOx: Select the GPIO GPIOx.
 *                  This parameter can be one of GPIOx( x can be from A to G).
 *
 ...
 *                  This parameter can be one of GPIO_PIN_x( x can be from 0 to 15).
 *
 * @retval    The output GPIOx GPIO_Pin value
 */
uint8_t GPIO_ReadOutputBit(GPIO_T* GPIOx, uint16_t GPIO_Pin)
{

    uint8_t ret;

    ret = (GPIOx->ODATA & GPIO_Pin) ? BIT_SET : BIT_RESET;

    return ret;
}

/*!
 * @brief     Reads the specified GPIO output data GPIOx
 *
 * @param     GPIOx: Select the GPIO GPIOx.
 *                  This parameter can be one of GPIOx( x can be from A to G).
 *
 * @retval    output data GPIOx value
 */
uint16_t GPIO_ReadOutputPort(GPIO_T* GPIOx)
{
    return ((uint16_t)GPIOx->ODATA);
}

/*!
 * @brief     Sets the selected data GPIOx bits
 *
 * @param     GPIOx: Select the GPIO GPIOx.
 *                  This parameter can be one of GPIOx( x can be from A to G).
 *
 * @param     GPIO_Pin : specifies GPIO_Pin to be written.
 *                  This parameter can be any combination of GPIO_PIN_x( x can be from 0 to 15).
 *
 * @retval    None
 */
void GPIO_SetBit(GPIO_T* GPIOx, uint16_t GPIO_Pin)
{
    GPIOx->BSC = (uint32_t)GPIO_Pin;
}

/*!
 * @brief     Clears the selected data GPIOx bits
 *
 * @param     GPIOx: Select the GPIO GPIOx.
 *                  This parameter can be one of GPIOx( x can be from A to G).
 *
 * @param     GPIO_Pin : specifies GPIO_Pin to be cleared.
 *                  This parameter can be any combination of GPIO_PIN_x( x can be from 0 to 15).
 *
 * @retval    None
 */
void GPIO_ResetBit(GPIO_T* GPIOx, uint16_t GPIO_Pin)
{
    GPIOx->BC = (uint32_t)GPIO_Pin;
}

/*!
 * @brief     Writes data to the specified GPIO data GPIOx bit
 *
 * @param     GPIOx: Select the GPIO GPIOx.
 *                  This parameter can be one of GPIOx( x can be from A to G).
 *
 * @param     GPIO_Pin : Select specifies GPIO_Pin.
 *                  This parameter can be one of GPIO_PIN_x( x can be from 0 to 15).
 *
 *
 * @param     bitVal : specifies the value to be written to the GPIOx output data register
 *                     This parameter can be one of the following values:
 *                       @arg BIT_RESET: Reset the GPIOx GPIO_Pin
 *                       @arg BIT_SET  : Set the GPIOx GPIO_Pin
 *
 * @retval    None
 */
void GPIO_WriteBitValue(GPIO_T* GPIOx, uint16_t GPIO_Pin, uint8_t bitVal)
{
    if (bitVal != BIT_RESET)
    {
        GPIOx->BSC = GPIO_Pin;
...
/*!
 * @brief     Writes data to the specified GPIO data GPIOx
 *
 * @param     GPIOx: Select the GPIO GPIOx.
 *                  This parameter can be one of GPIOx( x can be from A to G).
 *
 * @param     portValue : specifies the value to be written to the GPIOx output data register.
 *
 * @retval    None
 */
void GPIO_WriteOutputPort(GPIO_T* GPIOx, uint16_t portValue)
{
    GPIOx->ODATA = (uint32_t)portValue;
}
 ...
 * @retval    None
 */
void GPIO_ConfigPinLock(GPIO_T* GPIOx, uint16_t GPIO_Pin)
{
...
/*!
 * @brief     Selects the GPIO GPIO_Pin used as Event output
 *
 * @param     portSource : selects the GPIO GPIOx to be used as source for Event output.
 *                         This parameter can be one of GPIO_PORT_SOURCE_x( x can be from A to E).
 *
 * @param     pinSource   specifies the GPIO_Pin for the Event output
...
/*!
 * @brief     Changes the mapping of the specified GPIO_Pin
 *
 * @param     remap : selects the GPIO_Pin to remap
 *                    This parameter can be one of the following values:
...
/*!
 * @brief     Selects the GPIO GPIO_Pin used as EINT Line
 *
 * @param     portSource : selects the GPIO GPIOx to be used as source for EINT line.

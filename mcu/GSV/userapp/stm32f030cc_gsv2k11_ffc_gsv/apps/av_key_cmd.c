...
{
    if(enable == 1)
        GPIO_WriteBitValue(GPIOA, LED0_Pin, BIT_RESET);
    else
        GPIO_WriteBitValue(GPIOA, LED0_Pin, BIT_SET);
}

void TxOutLedOut(uint8 index, uint8 enable)
{
    if(enable == 1)
        GPIO_WriteBitValue(GPIOA, LED1_Pin, BIT_RESET);
    else
        GPIO_WriteBitValue(GPIOA, LED1_Pin, BIT_SET);
}

void LogicLedOut(uint8 enable)
{
    if(enable == 1)
        GPIO_WriteBitValue(GPIOA, LED2_Pin, BIT_RESET);
    else
        GPIO_WriteBitValue(GPIOA, LED2_Pin, BIT_SET);
}
